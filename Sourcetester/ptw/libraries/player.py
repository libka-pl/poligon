...
class player(xbmc.Player):
    ...
    def run(self, title, year, season, episode, imdb, tmdb, url, meta):
        try:
            ...
            self.name = quote_plus(title) + quote_plus(' (%s)' % year) if self.content == 'movie' else quote_plus(title) + quote_plus(' S%01dE%01d' % (int(season), int(episode)))
...
class subtitles:
    def get(self, name, imdb, season, episode):
        try:
            ...
            log_utils.log('subtitles get fail', 'module')
            pass
