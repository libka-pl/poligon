...
class sources:
    ...
    def addItem(self, title):
        ...
        def sourcesDirMeta(metadata):
            ...
            allowed = ['icon', 'poster', 'fanart', 'thumb', 'clearlogo', 'clearart', 'discart', 'banner', 'title', 'year', 'tvshowtitle', 'season', 'episode', 'rating', 'plot', 'trailer', 'mediatype']
...
#                meta.pop("next", None)
                    ...
                    if quality in ["0"]:
                        source_4k = len(
                            [
                                ...
                                if e["quality"] == "4K" and e["debridonly"] == False
                            ]
                        )
                        source_1080 = len(
                            [
                                e
                                for e in self.sources
                                if e["quality"] in ["1440p", "1080p"]
                                and e["debridonly"] == False
                            ]
                        )
                        source_720 = len(
                            [
                        ...
                        source_sd = len(
                            [
                                e
                                for e in self.sources
                                if e["quality"] == "SD" and e["debridonly"] == False
                            ]
                        )
                    elif quality in ["1"]:
                        source_1080 = len(
                ...
                if (i / 2) < timeout:
                    try:
                        mainleft = [
                            ...
                            for x in threads
                            if x.is_alive() == True and x.getName() in mainsourceDict
                        ]
                        info = [
                            sourcelabelDict[x.getName()]
                            for x in threads
    ...
    def sourcesFilter(self):
        ...
        if quality in ["0"]:
            filter += [
                i
                for i in self.sources
                if i["quality"] == "4K" and not "debrid" in i and "memberonly" in i
            ]
        if quality in ["0"]:
            filter += [
                i
                for i in self.sources
                if i["quality"] == "4K" and not "debrid" in i and not "memberonly" in i
            ]
        ...
        if quality in ["0", "1"]:
            filter += [
                i
                for i in self.sources
                if i["quality"] == "1440p" and not "debrid" in i and "memberonly" in i
            ]
        if quality in ["0", "1"]:
            filter += [
                i
                ...
                if i["quality"] == "1440p"
                and not "debrid" in i
                and not "memberonly" in i
            ]

        if quality in ["0", "1", "2"]:
            filter += [
                i for i in self.sources if i["quality"] == "1080p" and "debrid" in i
            ]
        if quality in ["0", "1", "2"]:
            filter += [
                i
                for i in self.sources
                if i["quality"] == "1080p" and not "debrid" in i and "memberonly" in i
            ]
        if quality in ["0", "1", "2"]:
            filter += [
                i
        ...
        if quality in ["0", "1", "2", "3"]:
            filter += [
                i
                for i in self.sources
                if i["quality"] == "720p" and not "debrid" in i and "memberonly" in i
            ]
        if quality in ["0", "1", "2", "3"]:
            filter += [
                i
        ...
        if not captcha == "true":
            filter = [
                i
                for i in self.sources
                if i["source"].lower() in self.hostcapDict and not "debrid" in i
            ]
        ...
        if sort_source is "0":
            ...
            self.sources = sorted(self.sources,
                                  key=lambda d: (not d["provider"].startswith("external"), not d["provider"].startswith("pobrane"), language_order[d["language"]], quality_order[d['quality']], d["provider"]))
        if sort_source is "1":
            log("FanFilm.Sortowanie Sortuję według źródeł")
            self.sources = sorted(self.sources,
                                  key=lambda d: (not d["provider"].startswith("external"), not d["provider"].startswith("pobrane"), language_order[d["language"]], quality_order[d['quality']], d["source"]))
        ...
        try:
            if not HEVC == "true":
                self.sources = [
                    i
                    for i in self.sources
                    if not "HEVC" in i["label"] or not "265" in i["label"]
                ]
        except:
            pass
    ...
    def filter_duplicates(self):
        ...
        log_utils.log('Removed %s duplicate sources from list' % (len(self.sources) - len(filter)), 'module')
        return filter
    ...
    def sourcesResolve(self, item, info=False):
        try:
            ...
            try:
                ...
                headers = ""
            headers = (
                urllib.quote_plus(headers).replace("%3D", "=")
                if " " in headers
                else headers
            )
    ...
    def sourcesDirect(self, items):
        ...
        items = [
            i
            for i in items
            if ("autoplay" in i and i["autoplay"] == True) or not "autoplay" in i
        ]

        if control.setting("autoplay.sd") == "true":
            items = [
                i for i in items if not i["quality"] in ["4K", "1440p", "1080p", "HD"]
            ]
