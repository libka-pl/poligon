    ...
    if isinstance(doms, list):
        for i in range(len(doms)):
            ...
            try:
                ...
                log_utils.log("list_request failed dom: " + repr(i) + " - " + dom, 'module')
                pass
    else:
...
def agent():
    return "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.71 Safari/537.36 Edg/97.0.1072.62"
...
class Cfcookie:
    ...
    def get(self, netloc, ua, timeout):
        try:
            ...
            if self.cookie is None:
                log_utils.log(
                    "%s returned an error. Could not collect tokens." % netloc
                )
            return self.cookie
    ...
    def _get_cookie(self, netloc, ua, timeout):
        ...
        init = re.compile("setTimeout\(function\(\){\s*.*?.*:(.*?)};").findall(result)[
            0
        ]
