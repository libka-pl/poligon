    ...
    def searchMovie(self, title, year, aliases):
        try:
            ...
            try:
                ...
                url = [
                    i[0] for i in r if self.matchAlias(i[1], aliases) and (year == i[2])
                ][0]
            except:
                url = None
                pass
        ...
        try:
            ...
            if "tvshowtitle" in data:
                ...
                url = "%s/film/%s-season-%01d/watching.html?ep=%s" % (
                    self.base_link,
                    cleantitle.geturl(data["tvshowtitle"]),
                    int(data["season"]),
                    ep,
                )
            ...
            if "tvshowtitle" in data:
                ...
                links = client.parseDOM(
                    r, "a", attrs={"episode-data": ep}, ret="player-data"
                )
            else:
            ...
            for link in links:
                try:
                    if link.startswith("//"):
                        link = "https:" + link
                    host = re.findall(
                        "([\w]+[.][\w]+)$", urlparse(link.strip().lower()).netloc
                    )[0]
