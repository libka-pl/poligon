...
class source:
    def __init__(self):
        ...
        self.search_link = "https://xt7.pl/mojekonto/szukaj"  # post
        self.support_search_link = "https://xt7.pl/mojekonto/szukaj/{}"
...
#
#                self.session.post("https://xt7.pl/mojekonto/szukaj", data=data, headers=self.headers)
#
#                data = {"sort": "size"}
#
#                self.session.post(
#                    "https://xt7.pl/mojekonto/szukaj/1", headers=self.headers, data=data
#                )
#                r = self.session.post(
#                    "https://xt7.pl/mojekonto/szukaj/1", headers=self.headers, data=data
#                ).text
    ...
    def resolve(self, url):
        ...
        if autoxt7 == "false":
            ...
            ret = xbmcgui.Dialog().yesno(
                "xt7",
                "Chcesz wykorzystać transfer ze swojego konta xt7 aby odtworzyć tę pozycję?\n Aktualnie posiadasz: [B]%s[/B] transferu"
                % limit,
            )
            if not ret:
                return
        ...
        self.session.post(
            "https://xt7.pl/mojekonto/sciagaj", data=data, headers=self.headers
        )
