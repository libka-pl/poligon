...
class source:
    ...
    def do_search(self, search_string, title, localtitle, year, search_type):
        ...
        for title in titles:
            try:
                r = self.sess.get("https://ekino-tv.pl/search/qf/?q=%s"
                                  % str.lower(title).replace(" ", "%20"), allow_redirects=False)
#                r = client.request(
#                    "https://ekino-tv.pl/search/qf/?q=%s"
#                    % str.lower(title + " HD").replace(" ", "%20"),
#                    headers={"Cookie": cookies},
#                )
                r = client.parseDOM(r.text, "div", attrs={"class": "movies-list-item"})
                for row in r:
                    ...
                    if search_type == "/serie/":
                        if self.is_match(title_found, title):
                            return link
                    else:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        ...
        url = urlparse.urljoin(self.base_link, url) + f'season[{season}]+episode[{episode}]+'
        return url
#        r = client.request(url, headers={"Cookie": cookies})
...
#        return None
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            r = client.request(url, redirect=False, headers={"Cookie": cookies})
    ...
    def resolve(self, url):
        try:
            cookies = cache.cache_get("ekino_cookie")["value"]
            header = {"Cookie": cookies + "; Actisoft-Network=true",
                      'referer': self.base_link,
                      'user-agent': client.agent()}
            splitted = url.split('-')
            host = splitted[-1]
            video_id = url.replace('-' + host, '')

#            splitted = url.split("'")
#            host = splitted[1]
#            video_id = splitted[3]
            transl_url = urlparse.urljoin(self.base_link, self.resolve_link) % (
                host, video_id
            )
            r = self.sess.get(transl_url, headers=header, allow_redirects=False)
#            result = client.request(
#                url, redirect=False, headers={"Cookie": cookies + "; prch=true"}
#            )
            streams = re.findall(
                'href="([^"]+)"\s*target=".+?"\s*class=".+?"', r.text, re.DOTALL
            )[0]
            return streams
        except Exception as e:
            print(e)
