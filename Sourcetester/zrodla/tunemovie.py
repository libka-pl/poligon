        ...
        try:
            ...
            if not "tvshowtitle" in data:
                if "123movies.sc" in self.base_link:
                    ...
                    u = [
                        (
                            ...
                            client.parseDOM(i, "div", attrs={"class": "jt-info"})[0],
                        )
                        for i in r0
                    ]
                else:
                ...
                url = [
                    i[0]
                    for i in u
                    if check == cleantitle.get(i[1]) and data["year"] == i[2]
                ][0]
                ...
                if "123movies.sc" in self.base_link:
                    ...
                    u = [
                        (i[0][0], i[1][0]) for i in u if len(i[0]) > 0 and len(i[1]) > 0
                    ]
                url = [
                    i[0]
                    for i in u
                    if check in cleantitle.get(i[1])
                    and ("Season %s" % data["season"]) in i[1]
                ][0]
            ...
            if not episode == None:
                ...
                ep_url = client.parseDOM(
                    result, "a", attrs={"data-name": str(episode)}, ret="href"
                )[0]
                ...
                try:
                    ...
                    for i in range(3):
                        result = client.request(
                            url, post=post, XHR=True, referer=ref, timeout="10"
                        )
                        if not result == None:
                            break
                    ...
                    for n in range(3):
                        try:
                            ...
                            post = urlencode(post)
                            result = client.request(
                                url, post=post, XHR=True, referer=ref
                            )
                            ...
                            if type(src) is list:
                                ...
                                for i in src:
                                    sources.append(
                                        {
                                            "source": "gvideo",
                                            "quality": directstream.googletag(i)[0][
                                                "quality"
                                            ],
                                            "language": "en",
                                            "url": i,
