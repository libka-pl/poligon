...
class source:
    ...
    def contains_all_words(self, str_to_check, words):
        ...
        for word in words:
            if not self.contains_word(str_to_check, word) and not self.contains_word(
                str_to_check2, word
            ):
                return False
        return True
    ...
    def login(self):
        ...
        tok = re.findall("""['"]csrf-token['"]\s*content=['"](.+?)['"]""", result)
            ...
            for title in titles:
                ...
                for result in results:
                    ...
                    try:
                        ...
                        if (
                            self.contains_all_words(
                                cleantitle.normalize(cleantitle.getsearch(segostitle)),
                                words,
                            )
                            and year == rok
                        ):
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            grid = client.parseDOM(
                result.text, "div", attrs={"class": "watch-table__column"}
            )
            video_links = client.parseDOM(
                result.text,
                "div",
                attrs={"class": "watch-table__column"},
                ret="data-link",
            )
