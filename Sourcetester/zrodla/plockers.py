...
class source:
    def __init__(self):
        ...
        self.domains = [
            "www8.putlockers.fm",
            "putlocker.unblockit.ch",
        ]  # cloudflare'd: 'kat.mn', 'www2.putlockers.gs', 'putlockerfree.net', '123putlocker.io'
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            query = (
                "%s season %d" % (title, int(data["season"]))
                if "tvshowtitle" in data
                else title
            )
            query = re.sub("(\\\|/| -|:|;|\*|\?|\"|'|<|>|\|)", " ", query)
            ...
            r, self.base_link = client.list_request(
                self.base_link or self.domains, query
            )
            ...
            for p in _posts:
                try:
                    post = (
                        client.parseDOM(p, "a", ret="href")[1],
            ...
            if "tvshowtitle" in data:
                ...
                sepi = "season-%1d/episode-%1d.html" % (
                    int(data["season"]),
                    int(data["episode"]),
                )
                ...
                link = [
                    i[0]
                    for i in posts
                    if cleantitle.get_title(title) in cleantitle.get_title(i[1])
                    and hdlr == i[2]
                ][0]
            ...
            try:
                ...
                try:
                    host = re.findall(
                        "([\w]+[.][\w]+)$", urlparse(url.strip().lower()).netloc
                    )[0]
            ...
            r = [
                (
                    ...
                    client.parseDOM(i, "p", attrs={"class": "server_servername"})[0],
                )
                for i in r
            ]
            if r:
