...
class source:
    def __init__(self):
        ...
        self.user_name = control.setting('nopremium.username') or 'fanfilm'
        self.user_pass = control.setting('nopremium.password') or 'fanfilm777'
    ...
    def login(self):
        ...
        self.authtoken = ''
        get_auth = cache.cache_get('nopremium_authtoken')
        ...
        if self.authtoken == '':
            if self.user_name and self.user_pass:
                ...
                if "authtoken" in response:
                    ...
                    cache.cache_insert('nopremium_authtoken', self.authtoken)
                    return True
        ...
        try:
            ...
            for title in titles:
                ...
                if "error" in search and search['message'] != '':
                    cache.cache_insert('nopremium_authtoken' , '')
                    return False
            ...
            log_utils.log("nopremium - Exception " + str(e), 'sources')
            return
    ...
    def sources(self, rows, hostDict, hostprDict):
        ...
        try:
            ...
            log_utils.log("nopremium - Exception ", 'sources')
            return sources
    ...
    def resolve(self, url):
        try:
            ...
            autonopremium = control.setting('autonopremium')
            ...
            if "file" in check_file:
                if "filesize" in check_file['file'] and "filename" in check_file['file']:
                    if autonopremium == 'false':
                        confirm = xbmcgui.Dialog().yesno("Pobieranie pliku" , "Pobieranie pliku " + check_file['file']['filename_full'] + "\n\nOd transferu zostanie odliczone " + self.recalculate(check_file['file']['chargeuser']) , yeslabel="Pobierz", nolabel="Anuluj")
                        if not confirm:
